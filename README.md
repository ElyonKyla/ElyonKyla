## Hi there - I'm Tania Veiga👋 






<h2 align="center">Backend Developer | AWS Certified Cloud Practitioner</h2>

<p align="center">

Backend developer with experience in enterprise software, data processing systems and cloud technologies.
I enjoy building reliable backend systems, working with structured data and integrating applications with databases and APIs.

Currently expanding my backend and cloud stack with Python and AWS.  
</p>

---

# 🛠 Tech Stack

<p>
<img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
<img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=oracle&logoColor=white"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/C%23-512BD4?style=for-the-badge&logo=dotnet&logoColor=white"/>
<img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white"/>

<img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white"/>
<img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white"/>

<img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white"/>
<img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black"/>



</p>

---
# ☁️ Certifications

<p align="center">

<a href="https://www.credly.com/earner/earned/badge/87b71a71-3e5c-4cd3-b7d9-f5b7a69774d4">
<img src="https://img.shields.io/badge/AWS-Certified%20Cloud%20Practitioner-orange?style=for-the-badge&logo=amazonaws"/>
</a>

<a href="https://www.credly.com/earner/earned/badge/cd667bb0-b759-45a2-9dca-66661b05ae89">
<img src="https://img.shields.io/badge/AWS%20Cloud%20Quest-Cloud%20Practitioner-blue?style=for-the-badge&logo=amazonaws"/>
</a>

<a href="https://www.credly.com/earner/earned/badge/a3dc5014-1a6e-44d4-9c26-dd99d478250d">
<img src="https://img.shields.io/badge/AWS%20Cloud%20Quest-Generative%20AI-purple?style=for-the-badge&logo=amazonaws"/>
</a>

<a href="https://sede.xunta.gal/cve?idcve=3eld4TADpjh0">
<img src="https://img.shields.io/badge/Xunta%20de%20Galicia-Developing%20on%20AWS-yellow?style=for-the-badge"/>
</a>

</p>


---

# 💼 Professional Background

Software engineer with experience across multiple areas of the software stack:

* Backend development with **Java and Python**
* Enterprise software development using **C / Pro*C and Oracle**
* Structured data processing systems
* ERP and business software development
* Software packaging and distributed deployments
* Systems administration and infrastructure support

---

# 🚀 Projects

## 🚗 Taller & Cars Listanco

Official website and vehicle inventory platform for a small automotive business, built with Angular and powered by Directus.

The website presents the workshop and its services, provides a public vehicle catalogue with detailed specifications and photo galleries, and includes information about vehicle imports from Germany.

**Tech**

Angular • TypeScript • SCSS • Directus • PostgreSQL • S3 • Railway • Netlify

**Live site**

[https://tallercarslistanco.es](https://tallercarslistanco.es)

**Repository**

[https://github.com/ElyonKyla/compraventa-coches](https://github.com/ElyonKyla/compraventa-coches)

**Status**

V1 is live on its production domain and verified on desktop and mobile. Vehicle content and ordered photo galleries are managed through Directus on Railway with restricted public read-only access. Production deployments are handled automatically by Netlify.

---
## 📦 Almacen — Mini ERP

Desktop ERP application designed to manage inventory, repairs and vehicle sales in a small business environment.

**Tech**

C# • WPF • Entity Framework

---

## 👥 Python_Admin_Web

User administration web application built with Flask.

**Features**

* CRUD user management
* JSON persistent storage
* Web administration interface

**Tech**

Python • Flask

---

## 🎬 CineProjectFS

Web application consuming movie APIs to display dynamic cinema listings.

**Tech**

JavaScript • REST APIs • HTML • CSS

*(currently under improvement)*

---

## 🚚 Tachograph Data Parser

Java application for parsing **digital tachograph binary files** based on European tachograph specifications.

**Tech**

Java • Data Parsing • Linux

*(project currently being redesigned)*

---

# 🔬 Current Development

## 📚 Books API (FastAPI)

Upcoming backend project designed to build a REST API for managing book data.

Planned stack:

Python • FastAPI • REST API • Authentication • Database integration





---

# 🌍 Location

Galicia, Spain 🇪🇸

---

<p align="center">
Thanks for visiting my profile!
</p>



